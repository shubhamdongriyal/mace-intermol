...
def get_swa(
    ...
    if args.loss == "virials":
        ...
        loss_fn_energy = modules.WeightedEnergyForceIntermolForceLoss(
            energy_weight=args.swa_energy_weight,
            forces_weight=args.swa_forces_weight,
            intermol_forces_weight=args.swa_intermol_forces_weight,
            mol_idxs=np.load(args.mol_idxs,allow_pickle=True)['arr_0'].tolist(),
        )
        logging.info(
            f"Stage Two (after {args.start_swa} epochs) with loss function: {loss_fn_energy}, with energy weight : {args.swa_energy_weight}, forces weight : {args.swa_forces_weight}, intermol forces weight : {args.swa_intermol_forces_weight} and learning rate : {args.swa_lr}"
        )
    elif args.loss == "universal":
